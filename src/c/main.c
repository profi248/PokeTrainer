...
typedef enum {
    THRESHOLD_LOW, THRESHOLD_MEDIUM, THRESHOLD_MAX 
} activity_threshold_lvl_t;
...
static bool s_use_dynamic_step_thresholds = true;
static int s_static_base_step_threshold = 10000;
static int s_sunset_static_hour_threshold = 19;
static int s_sunrise_static_hour_threshold = 6;
...
static int get_activity_threshold(activity_threshold_lvl_t lvl) 
{
    int base_steps;

    if (s_use_dynamic_step_thresholds) {
        get_step_goal();
        base_steps = s_step_goal;
    } else {
        base_steps = s_static_base_step_threshold;
    }
...
static void display_step_count()
{
    ...
    if (thousands > 0) {
        snprintf(s_current_steps_buffer, sizeof(s_current_steps_buffer), "%d,%03d",
            thousands, hundreds);
    } else {
        snprintf(s_current_steps_buffer, sizeof(s_current_steps_buffer), "%d",
            hundreds);
    }
    ...
    // APP_LOG(APP_LOG_LEVEL_DEBUG, "thresholds: %d %d %d", get_activity_threshold(THRESHOLD_LOW),
    //                                                     get_activity_threshold(THRESHOLD_MEDIUM),
    //                                                     get_activity_threshold(THRESHOLD_MAX));
    // APP_LOG(APP_LOG_LEVEL_DEBUG, "step part: %d > %d", s_step_count, get_activity_threshold(THRESHOLD_MAX));


    if (s_step_count < get_activity_threshold(THRESHOLD_LOW)) { // <2500, basic
        if ((persist_read_int(NUM_POKE_PKEY) == 2) || (persist_read_int(NUM_POKE_PKEY) == 3)) { // case Bulbasaur
            ...
            s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE007);
            bitmap_layer_set_bitmap(s_poke_layer, s_poke_bitmap);
        } else if ((persist_read_int(NUM_POKE_PKEY) == 134) || (persist_read_int(NUM_POKE_PKEY) == 135) || (persist_read_int(NUM_POKE_PKEY) == 136) || (persist_read_int(NUM_POKE_PKEY) == 196) || (persist_read_int(NUM_POKE_PKEY) == 197) || (persist_read_int(NUM_POKE_PKEY) == 470) || (persist_read_int(NUM_POKE_PKEY) == 471) || (persist_read_int(NUM_POKE_PKEY) == 700)) { // case Eevee
    ...
    else if (s_step_count >= get_activity_threshold(THRESHOLD_LOW) && s_step_count < get_activity_threshold(THRESHOLD_MEDIUM)) {
        //>2500, first ev
    ...
    else if (s_step_count >= get_activity_threshold(THRESHOLD_MAX)) {
    ...
    if (s_step_count >= get_activity_threshold(THRESHOLD_MEDIUM)) {
        //>5000, only ev
        if (persist_read_int(NUM_POKE_PKEY) == 133) { // case Eeveelution
            ...
            APP_LOG(APP_LOG_LEVEL_DEBUG,
                "the eeveelution number is %d, so NUM_POKE_KEY IS %d",
                eeveelution[n], (int)persist_read_int(NUM_POKE_PKEY));
            gbitmap_destroy(s_poke_bitmap);
    ...
    void* context)
{
    ...
    if (trainer_tuple) {
        if (strcmp(trainer_tuple->value->cstring, "0") == 1) { // case female
            ...
            s_trainer_layer = bitmap_layer_create(
                GRect(PBL_IF_ROUND_ELSE(78, 59), PBL_IF_ROUND_ELSE(49, 43), 45, 54));
            ...
            layer_add_child(window_get_root_layer(s_main_window),
                bitmap_layer_get_layer(s_trainer_layer));
        } else { // case male
    ...
    if (poke_tuple) {
        if (strcmp(poke_tuple->value->cstring, "25") == 0) {
            ...
            persist_write_int(NUM_POKE_PKEY,
                25); // pikachu
                // APP_LOG(APP_LOG_LEVEL_DEBUG, "NUM_POKE_PKEY IS
                // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        } else if (strcmp(poke_tuple->value->cstring, "1") == 0) {
            destroyAllPoke();
            if (s_step_count < get_activity_threshold(THRESHOLD_LOW)) {
                s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE001);
                persist_write_int(NUM_POKE_PKEY, 1); // bulbasaur
                APP_LOG(APP_LOG_LEVEL_DEBUG, "step count is %d", s_step_count);
            } else if (s_step_count >= get_activity_threshold(THRESHOLD_LOW) && s_step_count < get_activity_threshold(THRESHOLD_MAX)) {
                s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE002);
            ...
            // APP_LOG(APP_LOG_LEVEL_DEBUG, "NUM_POKE_PKEY IS NOW %d",
        ...
        s_poke_layer = bitmap_layer_create(GRect(PBL_IF_ROUND_ELSE(29, 10),
            PBL_IF_ROUND_ELSE(44, 38), 55,
            55)); // era 46, 58
        bitmap_layer_set_bitmap(s_poke_layer, s_poke_bitmap);
        bitmap_layer_set_background_color(s_poke_layer, GColorClear);
        bitmap_layer_set_compositing_mode(s_poke_layer, GCompOpSet);
        layer_add_child(window_get_root_layer(s_main_window),
            bitmap_layer_get_layer(s_poke_layer));
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "AFTER IF NUM_POKE_PKEY IS %d",
        // (int)persist_read_int(NUM_POKE_PKEY));
    }
    ...
    if (enableWeather_tuple) {
        APP_LOG(APP_LOG_LEVEL_DEBUG, "WEATHER ENABLER TUPLE IS %d",
            (int)enableWeather_tuple->value->int32);
        persist_write_int(NUM_ENABLEWEATHER_PKEY,
            (int)enableWeather_tuple->value->int32);
    }
    ...
    if (persist_read_int(NUM_ENABLEWEATHER_PKEY) == 1) {
        ...
        if (temp_tuple && conditions_tuple) {
            ...
            // snprintf(conditions_buffer, sizeof(conditions_buffer), "%d", stampa);
            // //I'm printing to see if it's changing
            ...
            snprintf(weather_layer_buffer, sizeof(weather_layer_buffer), "%s, %s",
                conditions_buffer, temperature_buffer);
        ...
        if (weather_units_tuple) {
            if (strcmp(weather_units_tuple->value->cstring, "0") == 0) {
                ...
                persist_write_int(
                    NUM_UNITS_PKEY,
                    numfaren); // case faren
                    // APP_LOG(APP_LOG_LEVEL_INFO, "option:
                    // %d", (persist_read_int(NUM_UNITS_PKEY));
            }
        }
        ...
        if (custom_text_tuple) {
            snprintf(weather_layer_buffer, 15, "%s",
                custom_text_tuple->value->cstring);
            persist_write_string(STR_TEXT_PKEY, weather_layer_buffer);
            APP_LOG(APP_LOG_LEVEL_DEBUG, "weather buffer is %s, the tuple is %s",
                weather_layer_buffer, custom_text_tuple->value->cstring);
            text_layer_set_text(s_weather_layer, weather_layer_buffer);
        }
    }
}
...
static void battery_update_proc(Layer* layer, GContext* ctx)
{
    ...
    int width = (int)(float)(((float)s_battery_level / 100.0F) * PBL_IF_ROUND_ELSE(159.0F, 131.0F)); // remember to change the multiplier,
        // it must be equa to the bar size
...
static void prv_unobstructed_change(AnimationProgress progress, void* context)
{
    ...
    // APP_LOG(APP_LOG_LEVEL_DEBUG, "I PRINT BOUNDS: %d, UNBOUNDS : %d, RESBOUNDS:
    // %d", bounds_int, un_bounds_int, res_bounds);
...
static void main_window_load(Window* window)
{
    ...
    // NEW SDK
    UnobstructedAreaHandlers handler = { .change = prv_unobstructed_change };
    ...
    if (persist_read_int(NUM_NIGHTMODE_PKEY) == 0) {
        bitmap_layer_set_bitmap(s_background_layer, s_background_bitmap);
        APP_LOG(APP_LOG_LEVEL_DEBUG,
            "Day Background applied, because NUM_NIGHTMODE_PKEY is %d",
            (int)persist_read_int(NUM_NIGHTMODE_PKEY));
    } else {
        bitmap_layer_set_bitmap(s_background_layer, s_backgroundNight_bitmap);
        APP_LOG(APP_LOG_LEVEL_DEBUG,
            "Night Background applied, because NUM_NIGHTMODE_PKEY is %d",
            (int)persist_read_int(NUM_NIGHTMODE_PKEY));
    }
    ...
    s_time_font = fonts_load_custom_font(
        resource_get_handle(RESOURCE_ID_FONT_PERFECT_DOS_22));
    ...
    text_layer_set_text_alignment(
        s_date_layer,
        PBL_IF_ROUND_ELSE(GTextAlignmentCenter, GTextAlignmentCenter));
    ...
    s_date_font = fonts_load_custom_font(
        resource_get_handle(RESOURCE_ID_FONT_PERFECT_DOS_12));

    // Apply Font
    text_layer_set_font(s_date_layer, s_date_font);

    // Create temperature Layer
    s_weather_layer = text_layer_create(GRect(PBL_IF_ROUND_ELSE(20, 10),
        PBL_IF_ROUND_ELSE(110, 145), bounds.size.w, 25));
    ...
    text_layer_set_text_alignment(
        s_weather_layer,
        PBL_IF_ROUND_ELSE(GTextAlignmentLeft, GTextAlignmentLeft));
    ...
    s_battery_layer = layer_create(GRect(PBL_IF_ROUND_ELSE(9, 6), PBL_IF_ROUND_ELSE(106, 100),
        PBL_IF_ROUND_ELSE(159, 131), 2));
    ...
    s_step_icon_layer = bitmap_layer_create(
        GRect(PBL_IF_ROUND_ELSE(111, 90), PBL_IF_ROUND_ELSE(110, 147), 12, 12));
    ...
    case 25:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE025);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED PIKACHU BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 1:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE001);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED BULBASAUR BECAUSE NUM_POKE_PKEY
        // IS NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 2:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE002);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED IVYSAUR BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 3:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE003);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED VENUSAUR BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 4:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE004);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED CHARMANDER BECAUSE NUM_POKE_PKEY
        // IS NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 5:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE005);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED CHARMELEON BECAUSE NUM_POKE_PKEY
        // IS NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 6:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE006);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED CHARIZARD BECAUSE NUM_POKE_PKEY
        // IS NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 7:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE007);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED SQUIRTLE BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 8:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE008);
    ...
    case 16:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE016);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED PIDGEY BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 17:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE017);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED PIDGEOTTO BECAUSE NUM_POKE_PKEY
        // IS NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 18:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE018);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED PIDGEOT BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 19:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE019);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED RATTATA BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 20:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE019);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED RATICATE BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 41:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE041);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED ZUBAT BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 95:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE095);
    ...
    case 37:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE037);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED VULPIX BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 86:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE086);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED SEEL BECAUSE NUM_POKE_PKEY IS NOW
        // %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 87:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE087);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED DEWGONG BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    case 42:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE042);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED GOLBAT BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        break;
    default:
        s_poke_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_POKE025);
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "I ADDED NOTHING BECAUSE NUM_POKE_PKEY IS
        // NOW %d", (int)persist_read_int(NUM_POKE_PKEY));
        // APP_LOG(APP_LOG_LEVEL_DEBUG, "NUM_TRAINER PKEY IS %d",
        // (int)persist_read_int(NUM_TRAINER_PKEY));
        break;
    }
    s_poke_layer = bitmap_layer_create(GRect(PBL_IF_ROUND_ELSE(29, 10),
        PBL_IF_ROUND_ELSE(44, 38), 55,
